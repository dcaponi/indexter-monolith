...
app.use((req, res, next) => {
  const allowedOrigins = ['http://127.0.0.1:8080', 'http://localhost:8080', 'https://indexter.tech' /* other prod domains */];
  ...
  try {
    ...
    app.use((req, res, next) => {
      ...
      if (req.cookies && req.cookies.lit){
        try {
          jwt.verify(req.cookies.lit, process.env.JWT_SIGNING_SECRET || 'local_signing_only');
