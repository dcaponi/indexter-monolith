...
export default class MagicLinkRepository extends BaseRepository<MagicLink> {
    ...
    async create(item: MagicLink): Promise<MagicLink> {
        try {
            let r = await this.pc.query<MagicLink>('INSERT INTO indexter.magic_links(email) VALUES($1)', [item.email]);
            if (r.rowCount == 0) {
            ...
            throw new Error('Magic Link save not successful. Unable to retrieve link');
        } catch (e) {
            console.log(e);
            throw new Error('Magic Link save not successful');
        }
    ...
    async delete(id: string): Promise<boolean> {
        try {
            await this.pc.query<MagicLink>('DELETE FROM indexter.magic_links WHERE id = ($1)', [id]);
            return true
        } catch (e) {
            console.log(e);
    ...
    async deleteByEmail(email: string): Promise<Boolean> {
        try {
            await this.pc.query<MagicLink>(`DELETE FROM indexter.magic_links WHERE email = ($1)`, [email]);
            return true;
        } catch (e) {
            console.log(e);
            throw new Error('Magic Link deletion not successful')
    ...
    async findOne(id: string): Promise<MagicLink> {
        try {
            let r = await this.pc.query<MagicLink>('SELECT * FROM indexter.magic_links WHERE id = ($1)', [id]);
            return r.rows[0];
        } catch (e) {
            console.log(e);
            throw new Error('Magic Link retrieval not successful')
