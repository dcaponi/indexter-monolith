...
export default class DocsRepository extends OwnedESIndexBaseRepository<Doc | ESResult> {
    pc: ESClient;
    idx: string;
    ...
    async create(owner: string, item: Doc): Promise<ESResult> {
        let doc = await this.pc.index({ index: owner + "_" + this.idx, id: item.id, document: item });
        return doc.result;
    }

    async bulkCreate(owner: string, items: Doc[]): Promise<ESResult> {
        let ops = items.flatMap(doc => [{ index: { _index: owner+"_"+this.idx, _id: doc.id } }, doc])
