...
export class AtlassianImporter implements Importer {
  ...
  importDocuments = async (email: string): Promise<Doc[] | null> => {
    let creds = (await this.credsRepo.findByUser(email)).filter(
      cred => cred.source == this.importSource
    )[0];
    if (!creds) {
      return null;
    }
    ...
    try {
      ...
      let tenants: {id: string; url: string; name: string}[] = (
        await axios.get(
        ...
        await Promise.all(
          tenants.map(async t => {
            let [pageRes, blogRes] = await Promise.all([
              axios.get(
                `https://api.atlassian.com/ex/confluence/${t.id}/wiki/rest/api/content?expand=body.view&type=page`,
                {headers}
              ),
              axios.get(
                `https://api.atlassian.com/ex/confluence/${t.id}/wiki/rest/api/content?expand=body.view&type=blogpost`,
                {headers}
              ),
            ]);
  ...
  private refreshAccessToken = async (
    ...
    let r = await axios.post<Credentials>(
