...
const accountLinkingRoutes = (pc: PoolClient) => {
    ...
    const googleIntegrationController = new IntegrationController(googleIntegration, credentialsRepository);
    const atlassianIntegrationController = new IntegrationController(atlassianIntegration, credentialsRepository);
    const slackIntegrationController = new IntegrationController(slackIntegration, credentialsRepository);

    router
        .get('/code/google', jwtCookieAuth, googleIntegrationController.exchangeAuthCode)
        .get('/code/atlassian', jwtCookieAuth, atlassianIntegrationController.exchangeAuthCode)
        .get('/code/slack', slackIntegrationController.exchangeAuthCode)
        .get('/links', jwtCookieAuth, async (req: Request, res: Response, next: NextFunction) => {
