...
export default class CredentialsRepository extends BaseRepository<Credentials> {
  ...
  async create(item: Credentials): Promise<Credentials> {
    try {
      let r = await this.pc.query(
        'INSERT INTO indexter.creds(source, email, access_token, refresh_token, expiry_date, alt_user_id) VALUES($1, $2, $3, $4, $5, $6)',
        [
          item.source,
          item.email,
          item.access_token,
  ...
  async update(id: string, item: Credentials): Promise<Credentials> {
    try {
      let r = await this.pc.query(
        'UPDATE indexter.creds SET access_token = ($1), refresh_token = ($2), expiry_date = ($3) WHERE id = ($4)',
        [item.access_token, item.refresh_token, item.expiry_date, id]
      );
      if (r.rowCount == 0) {
        throw new Error('Credentials save not successful');
      }
  ...
  async findOne(id: string): Promise<Credentials> {
    try {
      let r = await this.pc.query<Credentials>(
        'SELECT * FROM indexter.creds WHERE id = ($1)',
        [id]
      );
      return r.rows[0];
    } catch (e) {
  ...
  async findByUser(email: string): Promise<Credentials[]> {
    try {
      let r = await this.pc.query<Credentials>(
        'SELECT * FROM indexter.creds WHERE email = ($1)',
        [email]
      );
      if (r.rowCount == 0) {
